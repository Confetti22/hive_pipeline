...
def cos_loss_topk():
    """
    need to be pasted from older code, is now rewrite as ContrastiveLoss class
    """

class ContrastiveLoss(nn.Module):

    def __init__(self,n_views,pos_weight_ratio=5,topk=False,only_pos=False, infonce=True,temperature = 0.07,):
        super().__init__()
        """
        update: 2026/02/24
        rewrite into a class and merge with infonce_loss 
    ...
    def forward(self,features):
        ...
        N = features.shape[0]
        base = torch.arange(int(N // self.n_views), device=device)
        labels = torch.cat([base for _ in range(self.n_views)], dim=0)
        ...
        if self.infonce:
            positives, negatives = pos_coses, neg_coses
            ...
            if self.topk:
                k_pos = min(int(self.n_views/4),1)
                k_neg = int(N/10)
            ...
            pos_weight = (self.pos_weight_ratio)/(self.pos_weight_ratio+1)
            neg_weight = (1)/(self.pos_weight_ratio+1)
            
            return pos_weight*pos_loss + neg_weight+neg_loss, pos_coses.mean().item(),neg_coses.mean().item()




def get_loss(cfg) -> ContrastiveLoss:
	
    return ContrastiveLoss(cfg.n_views, cfg.pos_weight_ratio, cfg.topk, cfg.only_pos, cfg.infonce, cfg.temperature)
